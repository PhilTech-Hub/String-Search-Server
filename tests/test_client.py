...
class TestStringSearchClientFull(unittest.TestCase):
    ...
    def test_send_query_empty_string(self, mock_socket_class):
        ...
        with patch("ssl.create_default_context") as mock_ssl_context:
            ...
            # Empty query should now raise ValueError - update test expectation
    ...
    def test_send_query_non_utf8(self, mock_socket_class):
        ...
        with patch("ssl.create_default_context") as mock_ssl_context:
            ...
            client = StringSearchClient(host=self.host, port=self.port, use_ssl=False)
            client.connect()
            # Test with valid Unicode that might have encoding challenges
            response = client.send_query("café")  # Valid Unicode with accent
            self.assertEqual(response, "STRING NOT FOUND")
            client.close()

    def test_connection_logging(self):
        """Test client connection logging."""
        # Client logs to stdout, so capture stdout
        ...
        try:
            ...
            # Client should log connection attempts and closures - updated for new format
            self.assertIn("Establishing plain text connection", logs)
            self.assertIn("Successfully connected", logs)
            self.assertIn("SSL/TLS: No", logs)
            self.assertIn("Connection closed successfully", logs)
        finally:
            sys.stdout = old_stdout

    def test_ssl_connection_logging(self):
        ...
        try:
            ...
            logs = sys.stdout.getvalue()
            # SSL connections should generate logs - updated for new format
            self.assertIn("Establishing secure SSL/TLS connection", logs)
            self.assertIn("SSL/TLS handshake completed successfully", logs)
